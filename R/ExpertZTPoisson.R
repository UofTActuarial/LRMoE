## Expert Function: Zero-Truncated Poisson
#' Expert Function: Zero-Truncated Poisson.
#'
...
#' @param mean.theta A vector of length \code{1}: Zero-Truncated Poisson mean parameters.
...
#' @keywords internal
#'
#' @export ExpertZTPoisson
ExpertZTPoisson = function(tl, yl, yu, tu, mean.theta)
{
  ...
  censor.idx = (yl!=yu)
  prob.log.yu = ifelse(yu[censor.idx]==Inf, 0, pztpois(yu[censor.idx], lambda = mean.theta, lower.tail = TRUE, log.p = TRUE))
  prob.log.yl = ifelse(ceiling(yl[censor.idx])-1<1, -Inf, pztpois(ceiling(yl[censor.idx])-1, lambda = mean.theta, lower.tail = TRUE, log.p = TRUE))
  ...
  expert.ll[!censor.idx,1] = ifelse(yu[!censor.idx]<1, -Inf, dztpois(yu[!censor.idx], lambda = mean.theta, log = TRUE) ) # exact likelihood

  # Compute loglikelihood for expert j, then for truncation limits t
  prob.log.tu = ifelse(tu==Inf, 0, pztpois(tu, lambda = mean.theta, lower.tail = TRUE, log.p = TRUE))
  prob.log.tl = ifelse(ceiling(tl)-1<1, -Inf, pztpois(ceiling(tl)-1, lambda = mean.theta, lower.tail = TRUE, log.p = TRUE))
  ...
  no.trunc.idx = (tl==tu)
  expert.tn[no.trunc.idx,1] = ifelse(tu[no.trunc.idx]<1, -Inf, dztpois(tu[no.trunc.idx], lambda = mean.theta, log = TRUE) )
  ###################################################################

  ###################################################################
  # Deal with exact zero case: Applicable to ztpoisson.
  zero.idx = (tu==0)
